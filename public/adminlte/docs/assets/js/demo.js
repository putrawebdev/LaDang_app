...
(function ($) {
    ...
    var $block = $("<select />", {
      class: noneSelected
        ...
        : "custom-select mb-3 text-light border-0 " +
          colors[0].replace(/primary-|navbar-/, "bg-"),
    });
  ...
  $container.append("<h6>primary Color Variants</h6>");
  var $primary_variants = $("<div />", {
    class: "d-flex",
  });
  $container.append($primary_variants);
  $container.append(
    createSkinBlock(
      primary_colors,
      function () {
        var color = $(this).find("option:selected").attr("class");
        var $body = $("body");
        primary_colors.forEach(function (skin) {
          $body.removeClass(skin);
        });

        var primary_color_class = color.replace("bg-", "primary-");

        $body.addClass(primary_color_class);
      },
      true
    )
  );

  var active_primary_color = null;
  $("body")[0].classList.forEach(function (className) {
    if (
      primary_colors.indexOf(className) > -1 &&
      active_primary_color === null
    ) {
      active_primary_color = className.replace("navbar-", "bg-");
    }
  });

  // $primary_variants.find('option.' + active_primary_color).prop('selected', true)
  // $primary_variants.removeClass().addClass('custom-select mb-3 text-light border-0 ').addClass(active_primary_color)
